...
/**
 * Follows the stock Battery Saver toggle (not a Pepito Tweaks entry - see
 * PLAN-perf-battery.md): when isPowerSaveMode() is true, offlines the
...
public class GotweaksBatterySaverReceiver extends BroadcastReceiver {
    ...
    private static void apply(final boolean powerSaveMode) {
        Log.d(TAG, "Battery Saver " + (powerSaveMode ? "on" : "off")
                ...
                + (powerSaveMode ? "1" : "0"));
        SystemProperties.set(PROP_CPU_CLUSTER_SAVER, powerSaveMode ? "1" : "0");
    }
}
